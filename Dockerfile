...
RUN --mount=type=cache,mode=0777,target=/geyser-grpc-plugin/target \
    ...
    if [ -z "$features" ] ; then \
      cargo build --release && cp target/release/libgeyser* ./container-output && cp target/release/jito-geyser-cli ./container-output; \
    else \
      cargo build --release --features "$features" && cp target/release/libgeyser* ./container-output && cp target/release/jito-geyser-cli ./container-output; \
    fi
